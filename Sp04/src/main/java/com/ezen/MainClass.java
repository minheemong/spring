...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		// 멤버 변수는 Battery 인터페이스의 레퍼런스 변수이기 때문에  nbatt 또는  cbatt1 가
		// 모두 setBattery 에 사용이 가능합니다
		
		// 배터리는 비어있고, 배터리하나가 동봉되어, 동봉된 배터리를 장착해야만 사용이 가능한 상태
		...
		// 이렇게 객체 생성시에 다른 객체의 존재 및 생성자로의 전달이 필수인상태!
		// 객체생성이 다른 객체에 의존하고 있는 상태!
		// 이를 객체 의존 이라고 부릅니다.
		// 그리고 필요 객체를 생성자의 전달인수로 넣어 주는 것을 의존주입(Dependency Injection)
		// 이라고 부릅니다.
		
		// 앞선 Sp03 프로젝트와 는 다르게, Sp04 예제는 스프링 컨테이너를 이용함으로써, 위의 의존주입
		// 및 객체조립이 훨씬 간단 간결  편리해집니다.
